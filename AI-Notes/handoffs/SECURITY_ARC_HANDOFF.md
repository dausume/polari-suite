...
- 🔑 2026-09-13 FULL PURGE #3 (ledger §26): all three home machines emptied of Polari after every repo was pushed; isle-core lost DNS afterwards → HIS RULE: the uninstall must hand back a fully working default Ubuntu (journal, reapply, proof before "complete", offline rescue) — contract note sent to isle-core; a `handback` audit ring is ours to build.
...
7. **Interfaces** (SECURITY_INTERFACES_PLAN sec-i-0/1 — BEGUN 2026-09-12 night, plan §13): the `security` module exists with the taxonomy rows, `SecurityControl` per system × scenario with provenance stock/qemu/polari, and the THREE SECURITY TOPOLOGY VIEWS (`/display/security-os|network|app`) as reach simulations (`/api/security/topology|simulate|compare`, modes stock|today|complain|enforce; selftest 19/19). Not yet booted in a container or seen in a browser; the drawing of `nodes`/`edges` is a frontend decision (D10). PLUS the threat simulations (`/display/security-threats`, panel `security-threat-sim` — the one new frontend component, his ask): 14 threats × scenario × mode with the blocking policy and the counterexample. 2026-09-13: ALL the plan's rows now exist as DERIVED rows (interfaces plan §14): OS (MacProfile, DacPolicy, PermissionGroup, HardwareTrial), Network (ProxyConfig, ProxySnippet, ServiceIdentity, FirewallRuleSet), App (TrustChannel, AuthzRule, ContentPolicy, BrowserPolicy), the AppSecurityRecord LEDGER (260 rows, blocking step per app), SecurityAuditRun (the live feed: `pol security os audit --post <core>`), SecurityProposal (the write-back pass: `pol security os propose --profile P --accept`). Seven screens. FINDING from the derivation: every internal cert in both cert manifests is EXPIRED — `pol cert renew`. 2026-09-13 later: certificate notices (`/api/security/notices`, the app-system-notice bar), the device INVENTORY (`pol deploy inventory <node> [--post]`, DeviceInventory rows) and SSH AS A VECTOR (SshCapability rows, `/api/security/ssh`, panels on /display/topology-isle, the audit's ssh ring, the password-guess threat). Findings from the three machines (ledger §25): isle-core accepts PASSWORDS and root-with-key over ssh, no fail2ban anywhere, no auto-renew on isle-core/econ-core, pol-core has no sshd. Remaining: observe hooks, snippet composition, the hardware-trial runner, the per-app tab; everything unseen in a browser until the images are rebuilt.
