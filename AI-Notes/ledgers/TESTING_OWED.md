...
18. **DOWNLOADS PAGE BUILT + OFFLINE PLAN STAKED (your
    ...
    **dl-1b (2026-08-23 evening, e82f8c4 on dev-dl-1): the page
    ...
    full bundle first.
    **dl-3/4/5 PLANNED 2026-08-24 (his asks, PLANNING ONLY):**
    AI-Notes/plans/DOWNLOADS_PAGE_PLAN.md — all-in-one
    `polari-complete` merged deb (dpkg-in-dpkg is impossible →
    true content merge w/ Provides/Conflicts), /downloads/apps
    per-module app debs AUTO-GENERATED by polari+isle-mesh on
    version-hash change (debs deleted client-side after
    store-UI install; cross-module overlaps factored into shared
    debs so nothing installs twice), /downloads/offline chunk
    page. 8 decisions listed for your ratify; dl-4 recommends
    the dynamic-modules merge first (its live-admit is the
    consumer).
