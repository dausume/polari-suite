...
- **D3 — DECIDED 2026-09-13 (his): desktop is an axis, not a role.** Any role can be headless (a headless core, a headless hardware tier) and the desktop task is installed only on the desktop variant. New role `control` (above). Still open: whether a headless variant's offline pool carries the desktop packages for a later switch (bigger image).
...
- **D6** Secure Boot (explained to him 2026-09-13: the firmware boots only software signed by a trusted key; Ubuntu's boot loader is signed by Microsoft's key that every PC carries): use Ubuntu's signed shim + kernel and add no kernel modules of our own, so images boot with Secure Boot on or off and we sign nothing (recommended) — or sign nothing and require it off. Awaiting his word.
...
- **D12** The `control` role: does it require the manager app's desktop half (a desktop control box) or is a headless control box (isle CLI over ssh only) a first-class variant too? (Recommended: both; the CLI is the substance.)
