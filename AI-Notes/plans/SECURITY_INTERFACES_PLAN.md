...
## 13. Built 2026-09-12 (his ask: "what are we implementing in DAC and MAC … account for what docker and qemu already give … topology-based simulations … three security topology views first")

- The module `security` exists (`modules/security/`, scaffolded by the standard): `SecurityDomain` (3), `SecurityArea` (13), `SecurityScenario` (4, mirrors `os-security/scenarios/*.yml` when the suite tree is beside the checkout, else an embedded mirror), `SecurityControl` (92: every protecting SYSTEM × scenario with provenance **stock / qemu / polari** and state today), `SecurityTopologyNode` / `SecurityTopologyEdge` (285 / 413: the three views for every scenario as rows). `custom/security_facts.py` = the systems with provenance (14 default caps, masked paths, builtin seccomp, docker-default, device ACL, bridge isolation, the docker group; sVirt, qemu user, the hypervisor, VFIO; the Polari rings), the scenario mirror, and `APPLIED_TODAY`. `custom/security_topology.py` = the three views (`os`, `network`, `app`) as reach attempts with a chain of decisions, `simulate(view, scenario, actor, mode)` and `compare(view, mode)` (sources lined up by ROLE across routes). Modes: stock | today | complain | enforce.
- API: `/api/security`, `/api/security/scenarios`, `/api/security/topology?view&scenario&mode`, `/api/security/simulate?view&actor&scenario&mode`, `/api/security/compare?view&mode`. Default scenario = this deployment's route (`POLARI_DEPLOY_ROUTE`).
- Screens: `/display/security` (overview: scenario in force, the systems with provenance, domains, areas), `/display/security-os`, `/display/security-network`, `/display/security-app` — each: this scenario's edges, a simulation for a typical actor today and under enforce, the cross-scenario comparison, the boundaries, the edge rows. Structured panels + configured tables only.
- Selftest 19/19 (stock docker blocks module load / mount / ptrace / userns; stock ALLOWS image write, raw socket, chroot and enforce removes them; today on the isle logs the image write, on the swarm it is still allowed; the host-bind read is DAC's; a guest cannot escape; lean's API is open to a visitor, full refuses; compare lines the backend up across routes).
- Not built: the drawing (the three views are rows and structured panels; a graph renderer for `nodes`/`edges` is a frontend piece — decision D10: extend `topology-graph-view` with a generic layered mode, or a new security-topology component), `APPLIED_TODAY` fed by the live audit instead of a hand-kept dict, the remaining sec-i-2..6 objects (TrustChannel, AuthzRule, ContentPolicy, ProxySnippet, HardwareTrial, AppSecurityRecord).

- **Threat simulations (same night, his ask: "threat analysis simulations that act on the topology and show animations of the threats going through the network and how/what they get blocked by … along with the counterexample of with what valid permissions or group they are able to go in through it").** `custom/security_threats.py`: 14 named threats, each a real edge of a view (actor, means, target) resolved per scenario and mode with `path` (actor → the consulted boundaries with provenance and decision → target) and `stops_at`, the `blocked_by` system and its `policy` line, and a `counter` (actor, group, means, chain, verdict; "no legitimate path on this route" for hardware on a swarm). `SecurityThreat` rows (45 across the scenarios today), `/api/security/threats?scenario&mode`, `/display/security-threats`. Frontend: ONE new panel, `security-threat-sim` (registered in generic-display-components; inputs path/scenario/mode): a red token crosses the boundaries lane and stops with an X at the blocking policy, a green token walks the counterexample lane; mode selector replays under stock / today / complain / enforce; reduced-motion shows the final frame. Type-checks (tsc). Not yet built into an image or seen in a browser.

- **Physical access (2026-09-13, his ask after the ISO decisions):** two new OS-domain systems, `secure-boot` (area `boot`, provenance stock: Ubuntu's signed chain) and `disk-encryption` (area `at-rest`, provenance polari: a build-time option), a scenario `physical` fact (secure_boot / disk_encryption / headless; the audit's `physical` ring overrides it later), a `physical access` actor in the OS view with three reach attempts (pull the drive and read it elsewhere; boot a live USB and read the files — Secure Boot does NOT stop this, only encryption does; plant a tampered boot loader/kernel — Secure Boot's job), two threats with counterexamples (`stolen-drive`: the owner with the passphrase; `tampered-boot`: a signed kernel from apt), two audit controls (`physical secure-boot` via mokutil/efivars, `physical disk-encryption` via lsblk). Enforce mode turns encryption on only for non-headless profiles (the ISO rule). Selftest 32/32.

## 14. Built 2026-09-13 — the remaining rows of the three domains, the ledger, the live feed, the write-back pass (his "build out remaining security views and functionality")

All DERIVED, never typed, each derivation in its own file under `modules/security/custom/`:
- **OS** (`security_os_rows.py`): `MacProfile` from the render manifests per scenario + the mode "today" (151 rows: per-app and node profiles, seccomp per kind, sVirt on the isle); `DacPolicy` declared per app per scenario with live columns filled when an audit run reports containers (140); `PermissionGroup` from the sudoers drop-ins' Cmnd_Alias lines + the docker/libvirt/kvm/hardware groups (12); `HardwareTrial` (class, empty).
- **Network** (`security_network_rows.py`): `ProxyConfig` from the template text (server names, upstreams, TLS versions, HSTS via ${HSTS_HEADER}, headers, rate limits; 5); `ProxySnippet` per app — all 60 ABSENT, the gap for sec-i-4 made visible; `ServiceIdentity` from BOTH cert manifests + ca/issued with openssl end dates — FINDING: every internal cert in both manifests is expired (−25 / −9 days); `FirewallRuleSet` per scenario and chain (rendered vs applied).
- **App** (`security_app_rows.py`): `TrustChannel` from the proxy templates (user→app, TLS) + the Keycloak realms (public clients asymmetric, confidential clients symmetric) + ServiceConnection / IsleProtocolPermit rows when a manager is present (19; no symmetric key on a user channel); `AuthzRule` as a view over accessControl's permission sets/roles and the CRUDE gate (today: gate off, no sets → "every caller may CRUDE" on lean); `ContentPolicy` per class from the typing (runtime only — a manager is needed); `BrowserPolicy` per frontend host (6, derived CSP, report-only first).
- **The ledger** (`security_ledger.py`): `AppSecurityRecord` — 260 rows (every module + fixed piece × scenario), 12 steps, steps complete / total, the first blocking step; today every isle app is blocked at `mac_enforced` (loaded in complain), every swarm app at `mac_loaded`. `/api/security/ledger`, the overview screen.
- **The live feed** (`security_audit_feed.py`): `SecurityAuditRun` rows from `pol security os audit --post <core>` (or `pol deploy audit <node> --post`); `applied_for()` derives system → mode from the posted controls (loaded + not enforcing → complain; ufw pass → live …) and `physical_from_controls()` the Secure Boot / encryption facts — the hand-kept APPLIED_TODAY is the fallback until a run is posted.
- **The write-back pass** (`security_proposals.py`, his question "did we confirm automation passes for creating security profiles per piece of functionality?"): `propose_from_groups(app, stanza, harvest groups)` → a `SecurityProposal`: writable paths (directory level), capabilities from the allow-list, seccomp additions, the AppArmor lines, and the accesses the stanza CANNOT express (image/host writes, SYS_ADMIN, mounting — never widened into; bytecode-cache writes ignored: read-only root is the fix). `pol security os propose --profile isle-app-<app> [--accept]` runs the harvest, prints the proposal, and with `--accept` writes the stanza into the manifest (conform + render follow). `POST /api/security/propose` does the same from a posted harvest. The loop is now: declare → render → observe (warn-only) → propose → accept → re-render.
- Screens: tables added to `/display/security-os|network|app`; the ledger on `/display/security`; `/display/security-proxy` and `/display/security-firewall` (plan §4/§7). Seven pages. Selftest 39/39; conform OK; 23 classes wired (feature block, server literal).
- Not built: the observe hooks (ContentPolicy violations, CSP report endpoint), snippet composition into the proxy, the hardware-trial runner (pkexec), the per-app "Security" tab (per-object display config: needs the `?object=` page pattern), and everything in a browser.
