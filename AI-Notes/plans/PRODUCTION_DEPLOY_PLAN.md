...
## 16. The credential vault — auto-generated secrets no AI or other user can read (his ask 2026-09-11)

_"Make a secure file that has sudo-only permissions… clear labelling and encryption… leave it so they can access it later, or write it down and delete the file." Then the purpose, stated: **"to ensure no AI or other user is able to access it."**_ That purpose rules the design: the barrier has to hold against an operator account that has sudo and docker, because that is exactly what an AI session on pol-core has today. File permissions alone do not hold against that; only material the machine never holds does.

**The principle: the human holds the only key, and it is never on the machine.**
- The vault is encrypted **to the operator's public key**, not with a passphrase typed on the server. `age` (BSD-3, in Ubuntu's repos) accepts an **ssh public key as a recipient**, so the operator's existing `~/.ssh/id_ed25519.pub` (already on the server as their authorized key) is the recipient, and the matching private key lives only on their laptop. Automation, root, or an AI running the deploy can WRITE the vault and can never READ it. Decryption happens off the machine: `pol security vault export` copies the `.age` file down; `age -d -i ~/.ssh/id_ed25519` opens it there. No passphrase is ever typed into a session an AI can see, no `--passphrase-file`, no env var.
- The **generation step never writes plaintext to disk**: the values are produced in memory by root, piped straight into `docker secret create` (the swarm's encrypted Raft store; `--autolock` keeps its unlock key off disk) and into the age stream; `.generated/security-setup.log` records only names and lengths, never values. The plaintext env files the setup shell writes today are the thing this removes; compose (no secret store) stays 0600 files and the guide says that route does not meet this purpose.
- **Who can still read the live values:** the services themselves (they must), and anyone who can `docker exec` into them or read the Raft store — i.e. anyone in the `docker` group or with root. So the guarantee is only as strong as the operator account split: an AI's or a helper's account on a server belongs to `polari-app` / `polari-remote` (the limited sudoers drop-ins) and **never to `docker` or unrestricted `sudo`** (ISLE_HARDENING_PLAN §2.1). On pol-core today the assistant account has both, so nothing on pol-core is hidden from it; the droplet is where this holds from the first run, if its operator account is created that way.
- **What the vault contains:** a labelled banner (`POLARI CREDENTIALS — <domain> — generated <date> on <host> — encrypted to <ssh key comment>; only that key opens this`), then one block per credential with purpose, where it is used, and how to rotate: Keycloak admin, Keycloak DB, PSC DB, MariaDB root, MinIO root + client, the apt signing passphrase when one exists, the CA key's location (never its bytes).
- **Location:** `/etc/polari/vault/polari-credentials-<domain>-<date>.age`, `root:root 0700/0600`, outside the repo, one file per generation, old ones marked superseded on rotation. Because the file is unreadable without the off-machine key, "keep it here" is safe by construction; the choices at the end of `pol prod apply` become: **keep** (default; export later), **export now** to a path or over scp, or **shred** (the operator confirms they exported first). The write-down-and-delete option survives as "export, then shred".
- **Recipients:** more than one public key may be named (two operators, a recovery key kept offline); `pol prod apply` refuses to generate credentials with no recipient rather than fall back to a passphrase, and the unattended mode takes `POL_PROD_VAULT_RECIPIENTS=<pubkey file>`.

**Verbs.** `pol security vault list|export <path>|shred|recipients` (sudo; none of them decrypt — the server cannot); `pol security vault verify` checks the file decrypts by re-encrypting a test block to the same recipient (round-trip without the private key is impossible, so verify only checks the recipient header and the file's integrity). `pol security rotate prod` writes a new vault and new swarm secrets in one step. `pol security status` reports: vault present + recipients, or generated-but-unvaulted (a failure), and whether any plaintext credential file still exists on disk (a failure on the swarm route).
...
Phase **prd-9**: age recipient flow + in-memory generation into swarm secrets + vault write + choices + verbs + status checks + the placeholder fix; proof on the fresh droplet = no plaintext credential anywhere on disk after apply (`grep -r` of the checkout, `/etc/polari`, docker's data dir for the generated values fails), the operator decrypts the vault on the laptop and logs into Keycloak with it, and an account in `polari-remote` without `docker` cannot read any value. His **D9**: age via the ssh key (recommended) or gpg? **D10**: does the assistant's account on the droplet get `docker`? (Recommended: no — deploys run through the sudoers drop-in's fixed verbs, which is what makes the guarantee real.)
