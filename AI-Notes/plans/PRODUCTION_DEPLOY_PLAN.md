...
- Register: every module has `tier: core|optional`. Core (16): polariapps appstore islemesh terms vpn isle_relay isle_guestnet reticulum mqttbridge grpcbridge hardwareapps suiteapps hwmap testing resources xr (grpcbridge core by his ruling, same day). Rule: a core module never requires an optional one (checked, holds). `pol modules list` shows the tier.
