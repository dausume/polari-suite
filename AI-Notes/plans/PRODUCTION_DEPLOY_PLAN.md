...
- Register: every module has `tier: core|optional`. Core (15): polariapps appstore islemesh terms vpn isle_relay isle_guestnet reticulum mqttbridge hardwareapps suiteapps hwmap testing resources xr. Rule: a core module never requires an optional one (checked, holds). `pol modules list` shows the tier.
