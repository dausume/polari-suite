...
## 20. prd-10 BUILT 2026-09-11 — the Textual guide

`polari-cli/tui/prodguide/`: `model.py` (the answers as one dataclass; derived pairings — profile → compose → stack → cert row → names, exposure address, images source; `problems()` = every constraint, `warnings()` = advice from the facts; the only writer of `.generated/prod-answers.env`), `facts.py` (everything from `pol prod facts` JSON, `--check-image` for registry verification), `app.py` (steps list with state glyphs, one panel per step, DataTables for addresses/DNS/plan, links from the provider catalogue, Verify button for a registry+tag, review table, double-press Apply, `pol prod apply --yes` streamed into a RichLog, status/vault buttons after), `app.tcss`. `pol prod guide` launches it when a python with textual exists (`pol prod tui-install`: pip --user, else a venv beside the checkout; get-polari.sh runs it as step 4b); `POL_PROD_TUI=whiptail` keeps the dialogs. Bash keeps doing the work; the TUI produces answers and shows the run. Proven headlessly with Textual's pilot: every gate blocks (empty domain, bad address, missing e-mail, pull with tag prod, odoo without full…), the plan table has 14 rows, the answers file is written, apply needs a second press (a first walk without that guard started a real apply on pol-core during the test — hence the confirm step and the `PRODGUIDE_DRY` guard for tests). Not yet seen by a human on the droplet.

## 21. Module tiers and image variants (his rule 2026-09-12) — BUILT, released as polari-v2026.09.12

His rule: "determine a difference between core modules — what Polari needs to work as a networking and overall app system — and optional modules like odoo, fets or household, which only certain users need. The main image needs the core modules to operate coherently; variant images: one `core`, the other `all-official` carrying all of our official modules."
- Register: every module has `tier: core|optional`. Core (17): polariapps appstore islemesh terms vpn isle_relay isle_guestnet reticulum mqttbridge grpcbridge scoring hardwareapps suiteapps hwmap testing resources xr (grpcbridge + scoring core by his rulings, same day: the scorecard is for everyone). Rule: a core module never requires an optional one (checked, holds). `pol modules list` shows the tier.
- Dockerfile `--build-arg POLARI_MODULE_SET=core|all`: the core image prunes optional module directories, keeps the register. Tags `<release>-core`, `<release>-all`, plain = all. Every other image of a release carries both suffixes (same content) so a stack pulls one tag. Guide: the registry's tag list sorts `-core` first (default), names the variant; the modules step lists both tiers; a warning names optional modules listed on a core image.
- What the core image exposed and what was fixed: (1) core code imported optional modules unconditionally — gRPC sidecar (grpcbridge) now guarded; `seed_upsert` moved from composition into moduleService (shim kept, 57 importers repointed); (2) the runtime image had no `git`; (3) `/app/modules` was not writable by the runtime uid → fetched modules now live at `POLARI_FETCHED_MODULES_DIR=/app/data/modules` (data volume: survives restarts), on PYTHONPATH, and `module_code_dir` searches it. Proof: a core-image instance with mealoptions absent → `POST /modules/mealoptions/fetch-admit` from GitHub → cloned to the data volume → admitted from its manifest → registrar `online`.
- Still open: a lint that forbids unconditional optional imports in core code (today it is a grep); the broken first push of `polari-v2026.09.11-core` was overwritten with a booting build (deleting a ghcr tag needs the delete:packages scope).
