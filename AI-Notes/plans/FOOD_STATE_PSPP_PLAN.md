...
| fsp-3 (slice BUILT 2026-09-01) | pH priors (mpa-1) + food_chemistry.py: HH speciation over VERIFIED pKa (citric Goldberg 2002; malic Dawson 1986), exact TA stoichiometry, buffer-capacity REFUSES (I5); tomato organic-acid claims (Agius 2018 PMC6046607, density assumption named) + **the acid-chain acceptance PASSES**: chop→simmer→reduce concentrates citric exactly ×(100/40), speciation at measured pH 4.31 answers the state-184 example; 21/21. REMAINDER: buffer model calibration, measured-TA rows, acids for more foods (one cited measurement per food) |
