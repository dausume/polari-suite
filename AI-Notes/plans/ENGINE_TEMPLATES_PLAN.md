...
- **Staging:** compose services `frontend`/`backend` in polari-rf-node/docker-compose.staging-nip.yml, `LOCAL_IP=<pol-core LAN address>`; backend restore ~150s; API `curl -k -H "Host: api.prf.<pol-core LAN address>.nip.io" https://localhost/...`; frontend https://prf.<pol-core LAN address>.nip.io. Bash cwd persists between calls.
