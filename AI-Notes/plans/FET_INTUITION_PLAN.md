...
| fi-4 | per-object surfaces, backfill, docs | **REDEFINED by Dustin 2026-08-27**: (a) FET-VALIDITY GATE — `cnt_scoring.fet_validity`: five characteristic-equation proofs (states traversed on a rising sweep, gate modulation ≥ 1 decade, monotone Id(Vg), SS measurable, output saturation exists); any failure or an underived model ⇒ score **0** (not low) with the failed proofs named; the live binding refuses so the generic engine agrees. (b) PER-FET COMPETITIVE PAGE — `cnt_compare`: `/compare` ranks every device (levelized, focus ◀, per-term gap to the leader), graph `cnt-device-compare`, one seeded `DisplayDefinition` per FET (`/display/cntfet-score-{device}`); comparator device `cnt-aligned-s1-lg30` seeded (unproven → 0 until derived). cntfet-home backfilled. | remaining: device rows' Tables/Graphs tabs (TableDefinition per device), library_report links |
