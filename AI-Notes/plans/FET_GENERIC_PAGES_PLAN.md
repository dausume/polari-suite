...
| cell arc | ✅ BUILT (2026-08-31, Dustin's directive) | general cell pages + **CellFETConfiguration** (the cell×FET pairing as an OBJECT — 27 cells × every seeded device; uncharacterized = row + fill affordance, absence-as-data): `cnt_cell_pages.py` — `/api/fet/cell/{cell}/summary` (cell-summary/1: identity, proof, configuration index, OPEN-SOURCE SAMPLES flagged = cell ∧ device proven-free ∧ characterized), `/api/fet/cellcfg/{cell}/{device}/summary` (score vs THAT FET's limits + leakage states + acts + proof roll-up worst-of-both), `/api/fet/cells` catalogue; generic page `cell-detail` (`?object=<cell>`, `?device=` preselects) with the `cell-detail-panel` selector + logic/schematic/proof rows; cell surfaces alias under /api/fet. Target: solved live for ≥2 proven open-source samples (cinv, cnand2 on si-nmos-planar-90). selftest_cell_pages 8/8, summary 16/16. **cells-advance service** (his follow-up: no blank screens): `cnt_cell_advance.py` — GET `/api/fet/cells/advance` = the ladder report; POST `{"device"}` takes one device to the FIRST STEP (coarse 2×2 corner-grid library at its own Vdd, grid recorded on the run row + sequential DFF/latch), NEVER re-characterizes an existing run; sweep = `advance-cell-first-steps.sh` (idempotent). selftest_cell_advance. **BLOCK level** (his "move up" — the ladder is FET → cell → BLOCK → core → chip): `cnt_block_pages.py` — **BlockFETConfiguration** (block×FET rows, 4 blocks × devices), `/api/fet/block/{key}/summary` + `/api/fet/blockcfg/{key}/{device}/summary?timing=` + `/api/fet/blocks`; the config's `composition` lists the CellFETConfiguration rows beneath it (page links carry `?device=`) and cells gained `usedInBlocks` (the upward weave) — the explicit level-up object structure; generic page `block-detail` + `block-detail-panel`; readiness = every cell characterized (the advance sweep un-blanks blocks for free). selftest_block_pages |
