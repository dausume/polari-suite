...
## Seeing it: the three security topology views

The `security` module answers the three questions as three views, each a page of configured tables and structured panels: **OS** (what can a process touch on the machine, and which system stops it), **Network** (how do bytes get in, between and out) and **App** (who gets access to what, through which means). Every edge in a view is one reach attempt: a source, a target, the means, the chain of systems consulted in order with each one's provenance, and the verdict. Provenance is the point: *stock* means docker and the kernel already give this to every container (dropped capabilities, masked `/proc` paths, the builtin seccomp profile, the `docker-default` profile, network isolation); *qemu* means libvirt gives it to every guest (an unprivileged qemu, the per-VM sVirt profile, the hypervisor boundary, VFIO for one assigned device); *polari* means Polari renders or configures it (the profiles, the seccomp lists, the surface ring, the firewall chains, the remap, the proxy, the agent's doors, Keycloak, the sudoers groups). A simulation takes one actor, a scenario and a mode, and walks its reach hop by hop: *stock* (only what docker and qemu give), *today* (what is actually loaded on that scenario now), *complain* (a warn-only apply everywhere) and *enforce* (every Polari ring on). A comparison lines the same view up across the four scenarios by role, so the isle and the swarm sit side by side: on the swarm a module is the backend process and shares its profile, there are no guests and no devices, and the MAC ring is one node-wide profile; on the isle every app has its own, and hardware lives in a guest.
