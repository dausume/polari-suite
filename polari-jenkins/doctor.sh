...
if [ ! -f "$TGT_ENV" ] || ! grep -q '^DEPLOY_TARGETS="[^"]' "$TGT_ENV" 2>/dev/null; then
    ...
    _tprobe() {  # _tprobe <alias> → rows, from wherever the pipeline user's ssh config is
        if [ "${CTR_UP:-0}" = 1 ]; then docker exec "$CTR" bash -c 'a="$1"; out=$(ssh -o BatchMode=yes -o ConnectTimeout=8 "$a" "pol prod agent current" 2>&1); echo "rc=$?"; printf "%s\n" "$out"' _ "$1"
        else out=$(ssh -o BatchMode=yes -o ConnectTimeout=8 "$1" "pol prod agent current" 2>&1); echo "rc=$?"; printf '%s\n' "$out"; fi
    }
    ...
    for t in $(targets_list); do
        ...
        if [ "$rc" = 255 ]; then
            ...
            warn "target $t" "reached '$al' but the deploy agent did not answer (rc=$rc: $(printf '%s' "$body" | head -1))" \
                 "pol jenkins deploy authorize $t — it installs the key RESTRICTED to the target's agent (pol prod agent --path there must answer)"
        else
            rel="$(printf '%s\n' "$body" | sed -n 's/^release=//p')"; free="$(printf '%s\n' "$body" | sed -n 's/^free_gb=//p')"; st="$(printf '%s\n' "$body" | sed -n 's/^stack=//p')"
            ag="$(printf '%s\n' "$body" | sed -n 's/^agent=//p')"
            ok "target $t" "the deploy agent answers as the pipeline user (${ag:-?}); runs ${rel:-no release yet} (stack ${st:-none}), ${free:-?} GB free; hold=$hold$([ "$hold" = true ] && echo ' — only a person deploys (pol jenkins deploy '"$t"' --now)')"
        fi
    done
fi
