...
SC=""; MODE=""; RINGS="mac,network,host,dac"; DRY=false
while [ $# -gt 0 ]; do case "$1" in
    --scenario) SC="$2"; shift 2 ;; --complain) MODE=complain; shift ;; --enforce) MODE=enforce; shift ;;
...
if has network; then
    ...
    run bash "$OUT/docker-user.sh"
    run bash "$OUT/ufw.sh"
fi
if has host; then
    echo "== HOST: sysctl, auditd, units"
    run install -m 0644 "$OUT/sysctl.conf" /etc/sysctl.d/60-polari-os-security.conf && run sysctl -q --system && ok "sysctl applied"
    if command -v auditctl >/dev/null 2>&1; then
        run install -m 0640 "$OUT/audit.rules" /etc/audit/rules.d/60-polari-os-security.rules && run augenrules --load >/dev/null 2>&1 && ok "audit rules loaded" || warn "audit rules staged; augenrules --load failed"
    ...
    for U in isle-host-agent mesh-mdns polari-isle-push; do
        ...
        run install -d "/etc/systemd/system/$U.service.d" && run install -m 0644 "$OUT/systemd-hardening.conf" "/etc/systemd/system/$U.service.d/50-os-security.conf" && ok "$U hardened (restart to take effect)"
    done
    $DRY || systemctl daemon-reload
fi
if has dac; then
    echo "== DAC: ownership, docker daemon settings"
    run bash "$OUT/perms.sh"
