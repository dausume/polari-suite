...
MINIO_CONSOLE_URL=https://files.${PROD_DOMAIN}
MINIO_S3_URL=https://s3.${PROD_DOMAIN}
...
# prf/dps/oseb resolve to real production URLs; oseb is PRF's tech-tree route.
...
HUB_CONFIG_FILE="$GENERATED_DIR/pol-hub-runtime-config.prod.json"
HUB_MESH_URL="${ISLE_MESH_URL:-https://${PROD_DOMAIN}/docs.html}"
cat > "$HUB_CONFIG_FILE" << EOF
{
  "_comment": "PRODUCTION: Generated by prod-setup.sh — per-env project URLs for the Polari hub",
  "_generated": "$(date)",
  "_domain": "${PROD_DOMAIN}",
  "env": "prod",
  "links": {
    "prf": "https://prf.${PROD_DOMAIN}",
    "dps": "https://psc.${PROD_DOMAIN}",
    "mesh": "${HUB_MESH_URL}",
    "oseb": "https://prf.${PROD_DOMAIN}/tech-tree"
  }
}
EOF
