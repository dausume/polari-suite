...
ODOO_PG_ENV_FILE="$SCRIPT_DIR/pol-odoo-postgres/odoo-postgres.env"
ODOO_ENV_FILE="$SCRIPT_DIR/pol-odoo/odoo.env"
...
EOF
    ...
    # --- Odoo ERP Credentials (od-1) ---
    # odoo-postgres.env feeds the postgres service; odoo.env feeds the
    # odoo server — one shared secret, both written together (the minio
    # two-file precedent). SKIP-IF-EXISTS: the password is baked into the
    # odoo-db-data volume at first init — the credential-drift class from
    # the 2026-07-27 KC outage; rescue procedure in pol-odoo/README.md.
    # Knob: POLARI_ODOO_DB_PASS.
    echo "3b. Odoo ERP Credentials"
    echo "   Files: $ODOO_PG_ENV_FILE + $ODOO_ENV_FILE"

    if [[ -f "$ODOO_PG_ENV_FILE" && -f "$ODOO_ENV_FILE" ]]; then
        echo "   Already exist (kept — the password is baked into the"
        echo "   odoo-db-data volume; see pol-odoo/README.md to rotate)"
    else
        if [[ -n "${POLARI_ODOO_DB_PASS:-}" ]]; then
            ODOO_DB_PASS="$POLARI_ODOO_DB_PASS"
            echo "   Using provided Odoo DB password"
        elif [[ "$ENV" == "dev" || "$AUTO_MODE" == "true" ]]; then
            ODOO_DB_PASS=$(generate_password)
            echo "   Generated Odoo DB password"
        else
            read -sp "   Odoo DB password (Enter for random): " ODOO_DB_PASS
            echo ""
            if [[ -z "$ODOO_DB_PASS" ]]; then
                ODOO_DB_PASS=$(generate_password)
                echo "   Generated Odoo DB password: $ODOO_DB_PASS"
            fi
        fi

        mkdir -p "$(dirname "$ODOO_PG_ENV_FILE")" "$(dirname "$ODOO_ENV_FILE")"
        cat > "$ODOO_PG_ENV_FILE" << EOF
# Odoo PostgreSQL credentials (odoo-postgres service)
# Generated by setup-polari-security.sh — keep in sync with pol-odoo/odoo.env.
# Baked into the odoo-db-data volume at FIRST init; regenerating this file
# alone will NOT change the live DB password (drift trap — rescue in
# pol-odoo/README.md).
POSTGRES_USER=odoo
POSTGRES_PASSWORD=$ODOO_DB_PASS
# Keep the default 'postgres' maintenance DB; odoo_sim / odoo_ops are
# created via 'pol odoo init-db <sim|ops>'.
POSTGRES_DB=postgres
EOF
        cat > "$ODOO_ENV_FILE" << EOF
# Odoo server DB connection (official image entrypoint: HOST/PORT/USER/PASSWORD)
# Generated by setup-polari-security.sh — values mirror odoo-postgres.env
HOST=odoo-postgres
PORT=5432
USER=odoo
PASSWORD=$ODOO_DB_PASS
EOF
        chmod 600 "$ODOO_PG_ENV_FILE" "$ODOO_ENV_FILE"
        echo "   Created: $ODOO_PG_ENV_FILE"
        echo "   Created: $ODOO_ENV_FILE"
    fi
    echo ""
