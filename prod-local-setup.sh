#!/bin/bash
# ==============================================================================
# POLARI SUITE - PROD-LOCAL SETUP SCRIPT
# ==============================================================================
#
# This script sets up the prod-local environment with nip.io subdomain routing.
# It auto-detects your local IP and generates the necessary configuration.
#
# Run this script before starting the prod-local environment:
#   ./prod-local-setup.sh
#
# Then start the environment:
#   sudo docker compose -f docker-compose.yml -f docker-compose.prod-local-nip.yml --profile prod-local up -d --build
#
# ==============================================================================
...
# ==============================================================================
echo -e "${YELLOW}[1/4] Detecting local IP address...${NC}"
...
# ==============================================================================
echo ""
echo -e "${YELLOW}[2/4] Generating nginx configuration...${NC}"

mkdir -p "$GENERATED_DIR"

TEMPLATE_FILE="$PROXY_DIR/nginx.prod-local.conf.template"
OUTPUT_FILE="$GENERATED_DIR/nginx.prod-local.conf"
...
# ==============================================================================
echo ""
echo -e "${YELLOW}[3/4] Generating SSL certificates for *.${LOCAL_IP}.nip.io...${NC}"
...
EOF
    ...
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
        ...
        -addext "subjectAltName=DNS:*.${LOCAL_IP}.nip.io,DNS:${LOCAL_IP}.nip.io" \
        2>/dev/null
...
ENV_FILE="$GENERATED_DIR/.env.prod-local"

cat > "$ENV_FILE" << EOF
# ==============================================================================
# POLARI SUITE - PROD-LOCAL ENVIRONMENT
# ==============================================================================
# Generated by prod-local-setup.sh on $(date)
# IP Address: ${LOCAL_IP}
# ==============================================================================
...
{
  "_comment": "PROD-LOCAL: Generated by prod-local-setup.sh for nip.io testing",
  "_generated": "$(date)",
  "_ip": "${LOCAL_IP}",
...
echo -e "  ${YELLOW}sudo docker compose -f docker-compose.yml -f docker-compose.prod-local-nip.yml --profile prod-local up -d --build${NC}"
echo ""
if [ -f "$CA_CERT" ]; then
